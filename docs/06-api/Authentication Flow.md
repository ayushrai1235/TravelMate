...
TravelMate AI uses **Clerk** as the Identity Provider (IdP) for all authentication and user management. We do not store passwords or manage MFA logic ourselves.
...
1. **Clerk Hosted UI / SDK:** Handles login, signup, OTP, OAuth (Google), and password resets.
2. **Next.js Middleware:** Checks session cookies for page routing protection at the edge.
3. **Next.js BFF (API Routes):** Extracts the Clerk JWT, validates it, and proxies the request to FastAPI with an `X-User-ID` header.
4. **FastAPI Backend:** Trusts the `X-User-ID` header (because the BFF is the only entry point) or validates the Clerk JWT directly for defense-in-depth.
...
2. Next.js redirects to Clerk's Hosted UI (or renders Clerk `<SignIn />` component).
3. User authenticates via Google OAuth or Email + OTP.
4. Clerk issues a session cookie (`__session`) to the browser.
5. Clerk redirects back to TravelMate AI `/planner`.
...
2. Next.js API route uses Clerk SDK: `const { userId } = auth();`
...
For enhanced security (in case the FastAPI service is ever exposed directly), FastAPI can validate the Clerk JWT directly:

1. Next.js passes the Clerk JWT token in the `Authorization: Bearer <token>` header.
2. FastAPI uses `PyJWT` to fetch Clerk's JWKS (JSON Web Key Set).
...
Since Clerk holds the identity data, but we need user data in our PostgreSQL database for foreign keys (trips, preferences), we sync via Webhooks.

1. User signs up in Clerk.
2. Clerk sends `user.created` webhook to TravelMate AI.
